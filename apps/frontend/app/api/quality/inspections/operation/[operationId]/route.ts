  ...
  try {
    ...
    if (!isValidUUID(operationId)) {
      return NextResponse.json(
        { success: false, error: { code: 'INVALID_ID', message: 'Invalid Operation ID' } },
        { status: 400 }
      );
    }

    // Get operation inspection
    const result = await InProcessInspectionService.getByOperation(auth.orgId, operationId);
    ...
    return handleApiError(error, 'GET /api/quality/inspections/operation/:operationId');
  }
}
