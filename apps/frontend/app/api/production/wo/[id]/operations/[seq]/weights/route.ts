...
export async function POST(
  ...
  try {
    ...
    // Validate 1:1 components if applicable
    const oneToOneMaterials = woData.wo_materials?.filter((mat: any) => mat.one_to_one) || [];
    if (oneToOneMaterials.length > 0) {
      ...
      console.log('1:1 components detected - validation needed');
    }
    ...
    // Update work order current operation sequence if this was the current operation
    ...
    await supabase
      ...
      .insert({
        wo_id: woId,
        action: 'OPERATION_WEIGHTS_RECORDED',
        details: {
          operation_seq: seq,
          in_kg,
          out_kg,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        next_operation_seq: woData.current_operation_seq === seq ? seq + 1 : null
      }
    });
