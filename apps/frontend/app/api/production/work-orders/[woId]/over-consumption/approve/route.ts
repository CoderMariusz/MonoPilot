/**
 * API Route: Approve Over-Consumption Request
 * Story 04.6e: Over-Consumption Control
 *
 * POST /api/production/work-orders/:woId/over-consumption/approve
 * Approves an over-consumption request (Manager/Admin only)
 *
 * Security:
...
import { overConsumptionApprovalSchema } from '@/lib/validation/production-schemas'
...
export async function POST(
  ...
  try {
    ...
    const validation = overConsumptionApprovalSchema.safeParse(body)
    ...
    const { data: approvalRequest, error: reqError } = await supabase
      ...
      .select('id, status, wo_id, org_id, requested_qty, lp_id, wo_material_id')
      .eq('id', request_id)
      .eq('wo_id', woId)
    ...
    const { data: consumption, error: consumeError } = await supabase
      ...
      .insert({
        ...
        lp_id: approvalRequest.lp_id,
        consumed_qty: approvalRequest.requested_qty,
        consumed_by: user.id,
    ...
    console.error('Error in POST /api/production/work-orders/:woId/over-consumption/approve:', error)
