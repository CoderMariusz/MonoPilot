...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: traceData, error } = await supabase
      .rpc('vw_trace_forward', { input_lp_number: lp });
    ...
    console.error('Error fetching forward trace:', error);
    return NextResponse.json(
      { 
        success: false, 
        error: error instanceof Error ? error.message : 'Failed to fetch forward trace' 
      },
      { status: 500 }
    );
  }
