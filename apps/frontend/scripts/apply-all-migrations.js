...
/**
 * Apply All Migrations and Fix Categories
 * This script applies all migrations and fixes product categories
 */
...
async function applyAllMigrations() {
  console.log('🔄 Applying all migrations and fixing categories...\n');
  ...
  try {
    ...
    const migrationFiles = [
      ...
      '019_wo_materials_bom_snapshot.sql',
      '020_lp_reservations.sql',
      '020_wo_bom_snapshots.sql',
      '021_lp_compositions.sql',
      '021_reservations.sql',
      '022_pallets_compositions.sql',
      '022_pallets.sql',
      '023_sequential_routing_enforcement.sql',
      '023_wo_bom_snapshot_trigger.sql',
      '024_license_plates_stage_suffix_enhancement.sql',
      '024_qa_gate_enhancements.sql',
      '025_enhanced_trace_functions.sql',
      '026_fix_column_names.sql'
    ];
        ...
        if (fs.existsSync(migrationPath)) {
          ...
          for (const statement of statements) {
            if (statement.trim().length > 0) {
              try {
                // Skip complex statements that might cause issues
                ...
                if (statement.includes('INSERT INTO') || statement.includes('UPDATE') || statement.includes('DELETE')) {
                  // These will be handled by the data insertion section
                  continue;
                }
    ...
    // Step 4: Insert products with correct categories
    console.log('\n4. Inserting products with correct categories...');
    ...
    console.log('\n5. Verifying final distribution...');
    ...
    console.log('\n🎉 All migrations applied and data seeded successfully!');
