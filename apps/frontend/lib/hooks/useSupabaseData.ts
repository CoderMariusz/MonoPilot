    ...
    async function loadData() {
      try {
        ...
        const mapped: WorkOrder[] = (workOrders || []).map(wo => ({
          ...
          planned_qty: parseFloat(wo.planned_qty || wo.quantity || 0),
          completed_qty: parseFloat(wo.completed_qty || 0),
          uom: wo.uom,
          priority: wo.priority || 3,
          status: wo.status,
          scheduled_date: wo.scheduled_date || wo.scheduled_start,
          start_date: wo.start_date || wo.scheduled_start,
          end_date: wo.end_date || wo.scheduled_end,
          machine_id: wo.machine_id,
          production_line_id: wo.production_line_id || wo.line_id,
