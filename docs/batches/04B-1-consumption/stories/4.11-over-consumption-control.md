...
**Database:**
- `lib/supabase/migrations/040_add_allow_over_consumption.sql`
  - Adds `allow_over_consumption` column to production_settings (default: false)

**API:**
- `app/api/production/settings/route.ts`
  - Added `allow_over_consumption: false` to default settings creation

**Validation:**
- `lib/validation/production-schemas.ts`
  - Already had `allow_over_consumption: z.boolean().optional()`

**Frontend:**
- `app/(authenticated)/settings/production-execution/page.tsx`
  - Added toggle for "Allow Over-Consumption" setting
  - Descriptive help text explaining ON/OFF behavior

**Tests:**
- `e2e/story-4.11-over-consumption-control.spec.ts`
  - Tests for all ACs
...
|-----|-------------|----------|
| AC-4.11.1 | Setting Configuration | `040_*.sql`, `production-schemas.ts:19` |
| AC-4.11.2 | Behavior (4.12a) | Setting stored, used by 4.12a |
| AC-4.11.3 | Over-Production distinction | Documented in story |
| AC-4.11.4 | Settings API | `settings/route.ts:60` |
| AC-4.11.5 | Default: false | `040_*.sql:7`, `route.ts:60` |
| AC-4.11.6 | Settings UI | `page.tsx:250-270` |
| AC-4.11.7 | Dependencies | Ready for Story 4.12a |
