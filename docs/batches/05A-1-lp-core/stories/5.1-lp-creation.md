...
- **Then** display table with:
  ...
  - Quantity + UoM
  - Expiry Date
  - Status (badge: available=🟢, reserved=🟡, consumed=⚫, quarantine=🔴, shipped=🔵)
  - Location
  - Created Date
...
- [ ] **Extend `license_plates` table** (see tech-spec migration SQL):
  - Add `supplier_batch_number VARCHAR(50)`
  - Add `qa_status VARCHAR(20) DEFAULT 'pending'` with CHECK
  - Add `updated_by UUID REFERENCES users(id)`
  - Add `warehouse_id UUID REFERENCES warehouses(id)`
  - Update status CHECK to include 'reserved', 'merged'
...
### Integration Tests
- POST LP with FKs → verify record created
- POST LP duplicate number → verify unique constraint
- GET LP with filters → verify correct results
- DELETE LP → verify soft/hard delete

### E2E Tests
- Create LP via UI → verify in list
- Create LP with invalid data → see error message
- Delete LP → verify removed from list
...
- ✅ LP creation modal opens with all required fields
- ✅ LP number auto-generated in correct format
- ✅ LP created with status = 'available'
- ✅ Can list all LPs with filters
- ✅ Validation errors shown clearly
- ✅ Foreign key constraints enforced
- ✅ RLS policies tested (org isolation)
- ✅ E2E: create LP → see in list → verify all fields correct
