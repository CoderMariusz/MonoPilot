...
**Effort:** 5-6 hours
**Status:** Todo
**Updated:** 2025-11-28 (batch_number removed, supplier_batch only)
...
### AC 1: Supplier Batch Number Required
- **Given** creating LP
- **When** saving
- **Then** supplier_batch_number is required field
  - This is supplier's lot/batch number (for FDA traceability)
  - LP number itself plays the batch tracking role
...
- [ ] Verify expiry_date exists in license_plates table (FK from 5.1)
- [ ] Create endpoint filter: `?expiry_from=YYYY-MM-DD&expiry_to=YYYY-MM-DD`
- [ ] Implement FEFO sorting in GET /api/warehouse/license-plates queries
- [ ] Add query helper: `getExpiringSoonLPs(org_id, days=30)`
- [ ] Add validation: expired LP cannot move to 'available' status
- [ ] Exclude deleted LPs from expiry queries (deleted_at IS NULL)
