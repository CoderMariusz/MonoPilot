...
- **Then** stock_moves record created with:
  ...
  - quantity: Full LP quantity (or split qty if partial)
...
- **When** move completes
- **Then**:
  - LP.location_id updated to new location (optional: track via stock_moves instead)
  - LP still marked as 'available' (unless picked for WO)
  - Stock_moves table is source of truth for audit trail
...
### AC 6: Movement Type Specific Logic
- **Given** selecting different movement_type
- **When** choosing type
- **Then** validate movement constraints:
  - **receiving**: Source must be dock, destination must be storage
  - **put_away**: Source is receiving, destination is storage
  - **pick**: Source is storage, destination is production/shipping
  - **transfer**: Both source and destination are storage
