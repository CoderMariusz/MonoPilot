...
## 🚧 V2 MIGRATION - ADD MACHINE FLOW VISUALIZATION

**⚠️ V1 shows machine count (number), v2 needs machine flow (Mixer → Oven → Cooler)**

### **Build Location:**
```
✅ CREATE: apps/frontend/app/(authenticated)/settings-v2/production-lines/
✅ CREATE: apps/frontend/components/settings-v2/production-lines/
❌ DO NOT EDIT: app/(authenticated)/settings/production-lines/ (v1 frozen)
```

### **What to Add:**
- ✅ 2nd row: Machine flow visualization (Mixer → Oven → Cooler → Packing)
- ✅ Capacity column (units/hour)

### **What to Remove:**
- ❌ "Output Location" column (not in wireframe)
- ❌ Info banner (not in wireframe)

### **Can Reuse from V1:**
- ✅ `MachineSequenceEditor.tsx` (exists in modal, use for 2nd row display!)

### **Your Resources:**
- **Handoff:** `docs/2-MANAGEMENT/epics/current/01-settings/agent-handoffs/06-production-lines-machine-flow.yaml`
- **Wireframes:** `SET-018-production-line-list.md`, `SET-019-production-line-create-edit-modal.md`
