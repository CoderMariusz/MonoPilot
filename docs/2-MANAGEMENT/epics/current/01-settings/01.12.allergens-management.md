...
## 🚧 V2 MIGRATION - CRITICAL REWRITE

**⚠️ THIS STORY REQUIRES COMPLETE REWRITE (v1 is read-only, v2 needs CRUD + multi-language)**

### **Build Location:**
```
✅ CREATE: apps/frontend/app/(authenticated)/settings-v2/allergens/
✅ CREATE: apps/frontend/components/settings-v2/allergens/
❌ DO NOT EDIT: app/(authenticated)/settings/allergens/ (v1 frozen)
```

### **Why Complete Rewrite:**
- **V1 Architecture:** Read-only EU14 allergens (AllergenReadOnlyBanner)
- **V2 Architecture:** EU14 (locked) + Custom allergens (C01-C99) + Multi-language selector
- **V1 Missing:** Custom allergens CRUD, Language selector (EN/PL/DE/FR), Type badges

### **Your Resources:**
- **Handoff:** `docs/2-MANAGEMENT/epics/current/01-settings/agent-handoffs/02-CRITICAL-allergens-custom-rewrite.yaml`
- **Wireframe:** `docs/3-ARCHITECTURE/ux/wireframes/SET-020-allergen-list.md`
- **Migration Plan:** `docs/2-MANAGEMENT/EPIC-01-MIGRATION-PLAN.md` (section 2.8)
- **Master Prompt:** `docs/2-MANAGEMENT/epics/current/01-settings/MASTER-PROMPT-FOR-AGENTS.md`

### **Isolation Rules:**
- ❌ DO NOT use `AllergenReadOnlyBanner` (delete this concept!)
- ❌ DO NOT copy v1 components (all read-only, wrong!)
- ✅ MUST update `lib/validation/allergen-schemas.ts` (add custom type, multi-language fields)
- ✅ MUST update `lib/services/allergen-service.ts` (add createCustomAllergen method)

### **Compliance:**
- FR-SET-071: EU14 codes (A01-A14) - locked
- FR-SET-072: Multi-language (EN/PL/DE/FR)
- FR-SET-074: Custom allergens (C01-C99) - full CRUD
