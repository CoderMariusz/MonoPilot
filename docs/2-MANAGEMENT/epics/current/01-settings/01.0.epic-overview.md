# Epic 01 - Settings Module (Demo MVP)

**Epic ID:** 01
**Module:** Settings
**Status:** READY (Architecture + UX approved)
**Type:** Demo/MVP Subset
**Primary References:**
- PRD: `docs/1-BASELINE/product/modules/settings.md`
- Architecture: `docs/1-BASELINE/architecture/modules/settings.md`
- UX Wireframes: `docs/3-ARCHITECTURE/ux/wireframes/SET-001` to `SET-011`
- Split Proposal: `docs/2-MANAGEMENT/epics/current/01-settings/EPIC-01-SPLIT-PROPOSAL.md`

---

## Architecture Decisions (ADRs)

This epic implements or depends on the following ADRs:

| ADR | Decision | Stories |
|-----|----------|---------|
| ADR-011 | Module Toggle Storage | 01.1, 01.7 |
| ADR-012 | Role Permission Storage | 01.1, 01.5a, 01.6 |
| ADR-013 | RLS Org Isolation Pattern | 01.1, 01.5a, 01.5b, 01.6, 01.7 |

**ADR Locations:**
- `docs/1-BASELINE/architecture/decisions/ADR-011-module-toggle-storage.md`
- `docs/1-BASELINE/architecture/decisions/ADR-012-role-permission-storage.md`
- `docs/1-BASELINE/architecture/decisions/ADR-013-rls-org-isolation-pattern.md`

---

## Goal

Deliver a **minimal viable Settings module** focused on core Phase 1A functionality to demonstrate:
- Multi-tenant organization configuration with RLS foundation
- User management with 10-role permission system
- Module activation/deactivation toggles
- Onboarding wizard launcher (framework only)

This epic provides the **authentication and authorization foundation** required by all other epics.

---

## Non-Goals (Explicit)

**Out of Scope for Phase 1A:**
- Full onboarding wizard (steps 2-6: warehouse, location, product, work order, completion)
- Infrastructure setup (warehouses, locations, machines, production lines)
- Master data management (allergens, tax codes)
- API keys and webhooks
- Audit logs and security policies
- Notification settings
- Subscription and billing (Stripe)
- Import/export utilities
- User invitations (email integration)
- **User warehouse access restrictions (FR-SET-018) - Phase 1B, see 01.5b**

These are covered in the full **Epic 01** and will be implemented after the demo MVP.

---

## Delivery Strategy (Small Stories)

We intentionally keep stories small to reduce context load and enable rapid demo iteration:

1. **Foundation First:** Establish org context and RLS before any UI
2. **Shell + Guards:** Navigation structure and permission checking
3. **Wizard Framework:** State machine without full step implementation
4. **User CRUD:** Core user management with role assignment
5. **Module Toggles:** Enable/disable modules with dependency validation

**Constraints:**
- No external email integration (user creation without invitation flow)
- No Stripe integration (billing deferred)
- Wizard launcher shows framework, not complete wizard

---

## Story Index (Phase 1A Core)

| Story | Name | PRD FRs | UX | Dependencies | Complexity | Phase |
|------:|------|---------|-----|--------------|------------|-------|
| 01.1 | Org context + base RLS scaffolding | FR-SET-002 | (none) | -- | M | 1A |
| 01.2 | Settings shell: navigation + guards | FR-SET-030, FR-SET-031 | (implicit) | 01.1 | S | 1A |
| 01.3 | Onboarding wizard launcher | FR-SET-180, FR-SET-186, FR-SET-187 | SET-001 | 01.2 | M | 1A |
| 01.4 | Organization profile step | FR-SET-001, FR-SET-003, FR-SET-004, FR-SET-181 | SET-002, SET-007 | 01.3 | M | 1A |
| **01.5a** | Users: CRUD (MVP) | FR-SET-010, FR-SET-017 | SET-008, SET-009 | 01.6 | L | **1A** |
| **01.5b** | Users: Warehouse Access | FR-SET-018 | SET-009 | 01.5a, 01b.1 | M | **1B** |
| 01.6 | Role-based permissions (10 roles) | FR-SET-011, FR-SET-020 to FR-SET-031 | SET-011 | 01.1 | M | 1A |
| 01.7 | Module toggles | FR-SET-090 to FR-SET-097 | SET-022 | 01.6 | M | 1A |

**Phase 1A Stories:** 7 (01.1, 01.2, 01.3, 01.4, 01.5a, 01.6, 01.7)
**Phase 1B Stories:** 1 (01.5b)
**Estimated Effort Phase 1A:** 5-7 days (1 developer)

### Story Split Notice

Story **01.5** has been split into:
- **01.5a** - User Management CRUD (MVP) - Phase 1A
- **01.5b** - User Warehouse Access (FR-SET-018) - Phase 1B

See `EPIC-01-SPLIT-PROPOSAL.md` for details and `01.5.users-crud.md` for the split marker.

---

## Story Dependency Graph

```
01.1 (Org Context + RLS)
  |
  +---> 01.2 (Settings Shell)
  |       |
  |       +---> 01.3 (Wizard Launcher)
  |               |
  |               +---> 01.4 (Org Profile Step)
  |
  +---> 01.6 (Roles & Permissions)
          |
          +---> 01.5a (Users CRUD - MVP)
          |       |
          |       +---> 01.5b (Warehouse Access - Phase 1B) <---- Epic 01b.1
          |
          +---> 01.7 (Module Toggles)
```

**Critical Path Phase 1A:** 01.1 -> 01.6 -> 01.5a
**Critical Path Phase 1B:** 01.5a + Epic 01b.1 -> 01.5b

---

## PRD Requirements Coverage

### Covered in Phase 1A (MVP)

| Req ID | Requirement | Story |
|--------|-------------|-------|
| FR-SET-001 | Organization profile | 01.4 |
| FR-SET-002 | Multi-tenant isolation | 01.1 |
| FR-SET-003 | Timezone and locale | 01.4 |
| FR-SET-004 | Currency configuration | 01.4 |
| FR-SET-010 | User CRUD operations | **01.5a** |
| FR-SET-011 | 10-role permission system | 01.6 |
| FR-SET-017 | User deactivation/archiving | **01.5a** |
| FR-SET-020-029 | Role definitions (10 roles) | 01.6 |
| FR-SET-030 | Module-level permissions | 01.6 |
| FR-SET-031 | CRUD-level permissions | 01.6 |
| FR-SET-090 | Module activation/deactivation | 01.7 |
| FR-SET-091-096 | Individual module toggles | 01.7 |
| FR-SET-097 | Module dependency validation | 01.7 |
| FR-SET-180 | Setup wizard launcher | 01.3 |
| FR-SET-181 | Organization profile step | 01.4 |
| FR-SET-186 | Wizard progress tracking | 01.3 |
| FR-SET-187 | Skip wizard option | 01.3 |

### Covered in Phase 1B

| Req ID | Requirement | Story |
|--------|-------------|-------|
| FR-SET-018 | User warehouse access restrictions | **01.5b** |

### Deferred to Full Epic 01 (Phase 1B/1C/2/3)

| Req ID | Requirement | Reason |
|--------|-------------|--------|
| FR-SET-005 | Business hours configuration | Phase 1B |
| FR-SET-012-016 | User invitations, sessions, MFA | Email integration required |
| FR-SET-040-065 | Warehouses, locations, machines, lines | Phase 1B infrastructure (Epic 01b) |
| FR-SET-070-084 | Allergens, tax codes | Phase 1C master data (Epic 01c) |
| FR-SET-100-106 | Subscription & billing | Phase 3, Stripe required |
| FR-SET-110-116 | Multi-language support | Phase 1A but deferred |
| FR-SET-120-135 | API keys, webhooks | Phase 2 integrations |
| FR-SET-140-146 | Audit trail | Phase 1B |
| FR-SET-150-155 | Import/export | Phase 3 |
| FR-SET-160-163 | Notifications | Phase 2 |
| FR-SET-170-174 | Security policies | Phase 1B/3 |
| FR-SET-182-185, FR-SET-188 | Onboarding wizard steps 2-6 | Full epic 01 |

---

## External Dependencies (Cross-Epic)

None for Phase 1A - this epic is self-contained and provides the foundation for other epics.

**Phase 1B Dependency:**
- 01.5b requires Epic 01b.1 (Warehouse CRUD) - warehouses must exist before assigning access

**Provides to Other Epics:**
- `org_id` context for all tenant-scoped operations
- RLS policies for data isolation
- User authentication and role checking
- Module toggle validation

---

## Technical Notes

### Database Tables (Created/Modified in Phase 1A)

| Table | Action | Notes |
|-------|--------|-------|
| `organizations` | Create | Core org data + onboarding state |
| `users` | Create | User accounts with role assignment + `warehouse_access_ids` (NULL in MVP) |
| `roles` | Create | 10 predefined roles (ADR-012) |
| `modules` | Create | Module definitions (ADR-011) |
| `organization_modules` | Create | Org-specific module state (ADR-011) |

### RLS Policies (Established in 01.1)

All policies follow the ADR-013 pattern:
```sql
USING (org_id = (SELECT org_id FROM users WHERE id = auth.uid()))
```

### Key Services

- `organization-service.ts` - Org CRUD, onboarding state
- `user-service.ts` - User CRUD (no invitation flow) - **01.5a**
- `user-warehouse-service.ts` - Warehouse access (extends UserService) - **01.5b**
- `permission-service.ts` - Role checking, RBAC enforcement
- `module-settings-service.ts` - Module toggles, dependency validation

---

## Definition of Done (Epic 01 Phase 1A)

- [ ] Organization context established with RLS on all tables
- [ ] Settings shell with proper navigation and route guards
- [ ] Onboarding wizard launcher with skip/resume capability
- [ ] Organization profile form with validation
- [ ] User list with search, filter, pagination
- [ ] User create/edit modal with role assignment (no warehouse access in MVP)
- [ ] 10 roles with correct permission matrix enforced
- [ ] Module toggles with dependency validation
- [ ] All stories have unit tests (>80% coverage)
- [ ] Integration tests for API endpoints
- [ ] E2E smoke tests for critical flows

---

## Risk Assessment

| Risk | Impact | Mitigation |
|------|--------|------------|
| RLS policy gaps | High | Comprehensive integration tests |
| Permission misconfiguration | High | Test matrix for all 10 roles |
| Module dependency bugs | Medium | Validation tests for all toggle combinations |
| Onboarding state corruption | Low | State machine tests |
| 01.5b developed before 01b.1 | Medium | Explicit dependency in story |

---

## Success Metrics

- [ ] Demo: Create org, user, assign role, toggle modules
- [ ] All 10 roles enforced correctly
- [ ] Module toggles hide/show navigation items
- [ ] Onboarding wizard can be started, skipped, resumed
- [ ] Response time <500ms for all Settings APIs

---

## Handoff to DEV Agents

```yaml
epic: "01-settings"
epic_folder: docs/2-MANAGEMENT/epics/current/01-settings/
stories_count: 8  # 7 Phase 1A + 1 Phase 1B
stories_phase_1a: 7
stories_phase_1b: 1
start_story: "01.1.org-context-base-rls"
adrs:
  - ADR-011: Module Toggle Storage (modules + organization_modules tables)
  - ADR-012: Role Permission Storage (roles table with JSONB permissions)
  - ADR-013: RLS Org Isolation Pattern (users lookup pattern)
adr_folder: docs/1-BASELINE/architecture/decisions/
technical_notes: |
  - Foundation epic - establishes multi-tenant architecture
  - No external integrations (email, Stripe) in this epic
  - Focus on RBAC and module toggle framework
  - All RLS policies MUST use ADR-013 pattern
  - Roles table uses ADR-012 JSONB permissions
  - Module toggles use ADR-011 junction table (not module_settings)
  - Story 01.5 SPLIT into 01.5a (MVP) + 01.5b (Phase 1B)
  - 01.5b requires Epic 01b.1 (Warehouse CRUD) first
dependencies: []
split_stories:
  - original: "01.5.users-crud"
    replaced_by:
      - "01.5a.user-management-crud-mvp"
      - "01.5b.user-warehouse-access-phase1b"
```

---

## Version History

| Version | Date | Changes | Author |
|---------|------|---------|--------|
| 1.0 | 2025-12-15 | Initial epic overview for demo MVP | ARCHITECT-AGENT |
| 1.1 | 2025-12-15 | Added ADR section and updated handoff | DOC-AUDITOR |
| 2.0 | 2025-12-16 | Split 01.5 into 01.5a + 01.5b, updated story index | ARCHITECT-AGENT |
