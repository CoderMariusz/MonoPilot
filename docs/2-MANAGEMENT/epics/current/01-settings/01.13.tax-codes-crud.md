...
## 🚧 V2 MIGRATION - ADD EFFECTIVE DATES

**⚠️ V1 missing effective_from/effective_to fields (FR-SET-083 compliance required)**

### **Build Location:**
```
✅ CREATE: apps/frontend/app/(authenticated)/settings-v2/tax-codes/
✅ CREATE: apps/frontend/components/settings-v2/tax-codes/
✅ UPDATE: lib/validation/tax-code-schemas.ts (add date fields)
✅ UPDATE: lib/services/tax-code-service.ts (add date filtering methods)
❌ DO NOT EDIT: app/(authenticated)/settings/tax-codes/ (v1 frozen)
```

### **What's Missing in V1:**
- ❌ No `effective_from` / `effective_to` fields
- ❌ No expiration indicators (✓, ⏰, ⌛)
- ❌ No Effective Date filter (has Country filter instead)
- ❌ No date overlap validation

### **What V2 Needs:**
- ✅ Effective Date column (DD/MM/YY-DD/MM/YY or "Ongoing")
- ✅ Expiration indicators (green ✓, orange ⏰, gray ⌛)
- ✅ Effective Date filter dropdown (Currently Active, Expires Soon, Expired, Future)
- ✅ Date range picker in modal
- ✅ Date validation (no overlaps, proper ordering)

### **Your Resources:**
- **Handoff:** `docs/2-MANAGEMENT/epics/current/01-settings/agent-handoffs/03-CRITICAL-tax-codes-effective-dates.yaml`
- **Wireframes:** `SET-021-tax-code-list.md`, `SET-021a-tax-code-create-modal.md`, `SET-021b-tax-code-edit-modal.md`
- **Migration Plan:** `docs/2-MANAGEMENT/EPIC-01-MIGRATION-PLAN.md` (section 2.9)

### **Compliance:**
- FR-SET-083: Effective date support (MUST implement)

### **Can Migrate from V1:**
- ✅ `SetDefaultDialog.tsx` (good component)
- ✅ `DeleteTaxCodeDialog.tsx` (good component)
- ✅ `TaxCodeRateBadge.tsx` (reusable)
