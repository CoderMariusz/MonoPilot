...
- GIVEN WO.planned_qty = 5000 AND by-product yield_percent = 2.5, WHEN calculated, THEN expected qty = 125.
- GIVEN WO.planned_qty = 0 OR yield_percent = 0, WHEN calculated, THEN expected qty = 0.
...
- GIVEN auto_create_by_product_lp = true AND BOM has 3 by-products, WHEN main output registered, THEN all 3 by-product LPs created in same transaction.
- GIVEN auto_create enabled, WHEN by-products created, THEN success toast shows count of created LPs.
- GIVEN auto_create enabled AND by-product qty = 0, WHEN main output registered, THEN by-product LP created with qty = 0.
...
- GIVEN manual flow, WHEN modal opens, THEN quantity pre-filled with expected qty.
- GIVEN manual flow, WHEN user clicks "Register By-Product", THEN LP created and by-product status updated.

### Zero Quantity Handling

- GIVEN by-product qty entered = 0, WHEN user confirms, THEN warning "By-product quantity is 0. Continue?" displays.
- GIVEN zero qty warning shown, WHEN user clicks "Confirm Anyway", THEN LP registered with qty = 0.
- GIVEN zero qty warning shown, WHEN user clicks "Skip By-Product", THEN by-product skipped, no LP created.
- GIVEN zero qty warning shown, WHEN user clicks "Cancel", THEN returns to form.

### Genealogy Linkage

- GIVEN by-product registered, WHEN genealogy queried, THEN by-product LP has same parent_lp_ids as main output LP.
- GIVEN main output has 2 parent LPs (from consumed materials), WHEN by-product registered, THEN by-product LP shows same 2 parents.
- GIVEN genealogy copied, WHEN viewing by-product LP detail, THEN parent LPs display with consumed quantities.

### Batch Number Generation

- GIVEN main batch = "B-2025-0156" AND product_code = "BRAN", WHEN by-product registered, THEN batch = "B-2025-0156-BP-BRAN".
- GIVEN batch auto-generated, WHEN user edits batch, THEN edited value saved.
- GIVEN batch number max length = 50, WHEN generated batch exceeds limit, THEN truncated appropriately.

### By-Products Section UI

- GIVEN user on output registration page, WHEN page loads, THEN By-Products section displays below output history.
- GIVEN no by-products defined in BOM, WHEN section renders, THEN "No by-products defined for this WO" message displays.
- GIVEN 2 of 3 by-products registered, WHEN section renders, THEN 2 show "Registered" status, 1 shows "Not Registered".
- GIVEN by-product partially registered, WHEN progress shown, THEN progress bar reflects actual/expected percentage.

### Multi-tenancy

- GIVEN User A from Org A, WHEN requesting by-products, THEN only Org A data returned.
- GIVEN User A from Org A, WHEN attempting to register by-product for Org B WO, THEN 404 Not Found returns.
...
- [ ] Migration: Add by-product columns to production_outputs
- [ ] Migration: Add is_by_product column to license_plates
- [ ] RLS policies for by-product operations
- [ ] By-product service (`lib/services/by-product-service.ts`)
- [ ] Zod schemas (`lib/validation/by-product-schemas.ts`)
- [ ] API: POST /api/production/outputs/by-products
- [ ] API: GET /api/production/work-orders/:id/by-products
- [ ] ByProductsSection component
- [ ] RegisterByProductModal component
- [ ] ByProductCompletionModal component
- [ ] ZeroQtyWarningModal component
- [ ] Hook: useByProductRegistration
...
- [ ] Expected by-product qty displays correctly from BOM yield_percent
- [ ] Auto-create setting creates all by-product LPs on main output registration
- [ ] Manual entry flow allows custom quantities
- [ ] By-product LPs created with is_by_product = true
- [ ] Batch number auto-generated in correct format
- [ ] Genealogy copied from main output to by-product LPs
- [ ] Zero qty warning displays and allows confirmation
- [ ] By-Products section shows progress and status correctly
