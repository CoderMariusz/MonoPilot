...
### Required from Epic 01a (Settings Demo Subset)

Epic 02 requires ONLY the foundation from Epic 01a - no full Settings module needed.

| Dependency | Story | What It Provides | Status |
|------------|-------|------------------|--------|
| `organizations` table | 01a.1 | Multi-tenancy foundation, org_id FK for all tables | **REQUIRED** |
| `users` table | 01a.1 | Audit fields (created_by, updated_by) | **REQUIRED** |
| `roles` table seeded | 01a.1 | RBAC enforcement (Production Manager, Admin) | **REQUIRED** |
| Base RLS policies | 01a.1 | org_id isolation, auth.uid() resolution | **REQUIRED** |
| `getOrgContext()` helper | 01a.1 | Utility function for RLS queries | **REQUIRED** |

**Prerequisite**: Complete Epic 01a.1 (Org Context + Base RLS) before starting Epic 02.
...
|-------|-------|-----------|
| `allergens` | **02.3** | Static EU 14 regulatory data - seeded via migration, no UI needed initially |
...
**Epic 02 CAN proceed immediately once Epic 01a.1 is complete.**

```
Epic 01a.1 (Org Context + RLS)
    |
...
**No Full Epic 01 (Settings) required** - Epic 01a subset is sufficient.
...
- Epic 01a.1 ONLY (Org Context + Base RLS)
...
| 02.3 | Product Allergens | FR-2.4, FR-2.28 | TEC-010, TEC-012 | 02.1, 01a.1 | M | Ready |
...
| 02.8 | Routing Operations | FR-2.41, FR-2.43, FR-2.44, FR-2.45, FR-2.48 | TEC-008a | 02.7, 01a.1 | L | Ready |
| 02.9 | BOM-Routing Costs | FR-2.36, FR-2.37, FR-2.50-FR-2.53, FR-2.70, FR-2.72, FR-2.73, FR-2.74, FR-2.77 | TEC-013 | 02.5, 02.8 | M | Ready |
| 02.10a | Traceability Configuration | FR-2.60, FR-2.64 | TEC-016, TEC-014 | 02.1, 01a.1 | M | Ready |
...
| **053** | **Create allergens table (NEW - in 02.3)** |
| **054** | **Seed EU 14 allergens function (NEW - in 02.3)** |
