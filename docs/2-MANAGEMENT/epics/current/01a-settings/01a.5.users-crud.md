...
- Role assignment dropdown (10 roles from 01a.6)
...
- **01a.6** - Role-based permissions (required for role dropdown and enforcement)
...
### Database
- Uses existing `users` table with RLS policies from 01a.1
- `role_id` is a foreign key: `role_id UUID REFERENCES roles(id)`
- Queries must include `org_id` filter via RLS
- Status field: `active` (boolean) or `status` (enum: 'active', 'inactive')
...
- Reuse `permission-service.ts` from 01a.6 for role validation
...
const createUserSchema = z.object({
  ...
  role_id: z.string().uuid("Invalid role"), // Must reference roles(id)
});
```
