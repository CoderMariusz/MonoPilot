...
**Success Criteria:**
- ✅ Modal shows WO details clearly
- ✅ Pause reason dropdown populated from settings
- ✅ Operator can submit with or without reason
- ✅ Modal styled consistently with other modals
...
- ✅ paused_at timestamp recorded (server time)
- ✅ paused_by_user_id recorded for audit
- ✅ wo_pauses table records pause event
...
2. UPDATE work_orders:
   - status → 'in_progress'
   - resumed_at → current_timestamp (on WO? or in wo_pauses?)

3. UPDATE wo_pauses (last record):
   - resumed_at → current_timestamp
   - duration_minutes → (resumed_at - paused_at) / 60

4. RETURN: Updated WO with in_progress status
```

**Success Criteria:**
- ✅ Status transitions: paused → in_progress
- ✅ resumed_at timestamp recorded
- ✅ Pause duration calculated in wo_pauses
- ✅ No partial updates (atomic)
...
Pause History:
┌─────────────┬──────────┬──────────────────┬──────────┬─────────────┐
│ Reason      │ Notes    │ Paused At        │ Duration │ Resumed At  │
├─────────────┼──────────┼──────────────────┼──────────┼─────────────┤
│ Breakdown   │ Motor    │ 10:00 2025-11-28 │ 90 min   │ 11:30       │
│ Break       │ Lunch    │ 12:00 2025-11-28 │ 60 min   │ 13:00       │
│ Material W. │          │ 14:00 2025-11-28 │ 15 min   │ 14:15       │
└─────────────┴──────────┴──────────────────┴──────────┴─────────────┘
...
**Success Criteria:**
- ✅ Pause history table displays all pauses
- ✅ Duration calculated in minutes
- ✅ Total downtime calculated and displayed
- ✅ Breakdown by reason summarized

---

### AC-4.3.5: Pause Reason Configuration
**Given** admin is in Settings → Production Settings
...
- ✅ Dropdown populated from settings
- ✅ Disabled reasons not shown in dropdown
...
**Success Criteria:**
- ✅ POST /api/production/work-orders/:id/pause available
- ✅ POST /api/production/work-orders/:id/resume available
- ✅ Both enforce org_id isolation
- ✅ Both enforce authentication (401 if not logged in)
...
|------|-----------|-----------|-------|
| Production Manager | ✅ Yes | ✅ Yes | Primary user |
| Operator | ✅ Yes | ✅ Yes | Can pause own WOs |
| Planner | ❌ No | ❌ No | Planning only |
| Quality Manager | ❌ No | ❌ No | Quality only |
| Admin | ✅ Yes | ✅ Yes | Full access |

**Success Criteria:**
- ✅ Role check on both API endpoints
- ✅ 403 error if unauthorized
- ✅ Pause/Resume buttons hidden in UI for non-authorized roles
...
**Success Criteria:**
- ✅ Button visibility controlled by production_settings.allow_pause_wo
- ✅ Default: allow_pause_wo = TRUE
...
- ✅ Success toast: "Work order paused successfully"
- ✅ Error toast with retry option
- ✅ Modal auto-closes on success (1 second delay)
...
**Success Criteria:**
- ✅ Paused WOs visible in dashboard active list
- ✅ Resume button available
- ✅ Pause reason/duration displayed
...
- [ ] Task 10: Unit tests - PauseResumeService
  ...
  - [ ] Subtask 10.2: Test status transition: in_progress → paused
  - [ ] Subtask 10.3: Test resumeWorkOrder with valid WO
  - [ ] Subtask 10.4: Test status transition: paused → in_progress
