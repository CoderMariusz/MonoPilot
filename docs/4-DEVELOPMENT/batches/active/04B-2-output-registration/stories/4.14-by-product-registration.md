...
- `lib/supabase/migrations/042_add_byproduct_fields_to_wo_materials.sql` - Migration for by-product columns
- `lib/services/byproduct-service.ts` - By-product registration service
- `app/api/production/work-orders/[id]/by-products/route.ts` - API endpoint
- `components/production/ByProductRegistrationDialog.tsx` - Sequential by-product dialog
- `components/production/OutputRegistrationModal.tsx` - Integration with main output flow
- `e2e/story-4.14-byproduct-registration.spec.ts` - E2E tests

### Key Implementation Details:
- By-product LPs use "BP-" prefix
- Manual operator entry (not sequential consumption)
- Expected qty shown as reference (yield_percent × main_output_qty)
- Skip This / Skip All buttons for optional registration
- Genealogy links to same parent LPs as main output
