...
- **Then** complete within 8 seconds
...
- Performance: 1000 genealogy records traced in <8s
```
...
- **Performance Optimization (if needed)**: If genealogy tables exceed 100k+ records and SLA is not met, implement a **materialized view** (`mv_lp_genealogy_flat`) that pre-computes genealogy paths. This converts expensive recursive CTEs to fast index lookups. Refresh on schedule (e.g., hourly) or on-demand after genealogy operations. This is optional and should only be implemented if performance testing shows <8s SLA is not achievable with recursive CTEs + indexes alone.
