...
**Effort:** 5-6 hours
**Status:** Todo
**Updated:** 2025-11-28 (added 'merged', 'deleted' status + soft delete)
...
- **Then** valid transitions are:
  ...
  - `available` → `merged` (when merging into another LP) ⭐ NEW
  ...
  - `merged` → IMMUTABLE (cannot change; archive only) ⭐ NEW
  - Any status → `shipped` (final state)
  - Any status → `deleted` (soft delete, but NOT final) ⭐ NEW
  - `deleted` → Any status (can be restored by hard delete only)
...
- **Then** show status with color badges:
  ...
  - quarantine = 🔴 Red
  - shipped = 🔵 Blue
  - merged = 🟣 Purple (immutable, archived) ⭐ NEW
  - deleted = ⚪ White/Gray (soft deleted, can restore) ⭐ NEW
