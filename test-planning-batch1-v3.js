...
async function runTests() {
  ...
  try {
    ...
    console.log('\n=== PLANNING DASHBOARD BATCH 1 (V3 - FIXED) ===\n');
    ...
    // Test 4: PO Pending Approval KPI visible (now using correct selector)
    try {
      await page.goto('http://localhost:3000/planning', { waitUntil: 'domcontentloaded' });
      await page.waitForTimeout(500);
      const kpi = page.locator('[role="button"]').filter({ hasText: 'PO Pending Approval' }).first();
      await kpi.waitFor({ state: 'visible', timeout: 2000 });
      const text = await kpi.textContent();
      if (!text.includes('PO Pending Approval')) throw new Error('Text mismatch');
    ...
    try {
      ...
      await kpi.waitFor({ state: 'visible', timeout: 2000 });
      const text = await kpi.textContent();
      if (!text.includes('PO This Month')) throw new Error('Text mismatch');
    ...
    try {
      ...
      await kpi.waitFor({ state: 'visible', timeout: 2000 });
      const text = await kpi.textContent();
      if (!text.includes('TO In Transit')) throw new Error('Text mismatch');
    ...
    try {
      ...
      await kpi.waitFor({ state: 'visible', timeout: 2000 });
      const text = await kpi.textContent();
      if (!text.includes('WO Scheduled Today')) throw new Error('Text mismatch');
    ...
    try {
      ...
      await kpi.waitFor({ state: 'visible', timeout: 2000 });
      const text = await kpi.textContent();
      if (!text.includes('WO Overdue')) throw new Error('Text mismatch');
    ...
    try {
      ...
      await kpi.waitFor({ state: 'visible', timeout: 2000 });
      const text = await kpi.textContent();
      if (!text.includes('Open Orders')) throw new Error('Text mismatch');
    ...
    try {
      ...
      await feed.waitFor({ state: 'visible', timeout: 2000 });
    ...
    console.log('\nResults saved to batch1-results.json');
